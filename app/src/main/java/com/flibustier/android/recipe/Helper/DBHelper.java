...
import com.flibustier.android.recipe.Model.SearchResultItem;
...
public class DBHelper extends SQLiteOpenHelper
{
    ...
    public ArrayList<SearchResultItem> ingredients_selectRecipeByIngredientName(ArrayList<String> ingredientsName){
        ...
        Cursor cursor = db.rawQuery(String.format("SELECT recipeID, count(*) FROM INGREDIENTS WHERE %s GROUP BY recipeID", strNames), null);
        if (cursor != null) {
            while (cursor.moveToNext()) {
    ...
    public ArrayList<RecipeItem> recipes_SelectBest()
    {
        ...
        ArrayList<RecipeItem> allRecipes = new ArrayList<>();
        // Get all recipes data
        Cursor cursor = db.rawQuery("SELECT * FROM RECIPES ORDER BY likeCount DESC LIMIT 20", null);
        if (cursor != null)
        {
            while (cursor.moveToNext()) {
                allRecipes.add(new RecipeItem(
